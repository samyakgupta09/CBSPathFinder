...
class CBSState:
    ...
    def is_solution(self):
        ...
        state_set = set()
        for i in range(self._k):
            cur_path = self._paths[i]
            for j in range(len(cur_path)):
                cur_state = cur_path[j]
                if cur_state in state_set:
                    return False, (cur_state, cur_state.get_g())
                else:
    ...
    def successors(self):
        ...
        """
        children = []
        _, conflict_state = self.is_solution() 
        conflicting_agents = []
        for i in range(self._k):
            if conflict_state[1] < len(self._paths[i]):
                if self._paths[i][conflict_state[1]] == conflict_state[0]:
                    conflicting_agents.append(i)
        child1 = CBSState(self._map, self._starts, self._goals)
        child1._constraints = copy.deepcopy(self._constraints)
        child1.set_constraint(conflict_state[0], conflict_state[1], conflicting_agents[0])
        children.append(child1)
        child2 = CBSState(self._map, self._starts, self._goals)
        child2._constraints = copy.deepcopy(self._constraints)
        child2.set_constraint(conflict_state[0], conflict_state[1], conflicting_agents[1])
...
class CBS():
    def search(self, start):
        ...
        while OPEN:
            ...
            issol, _ = cur_node.is_solution()
            if issol:
                return cur_node._paths, cur_node._cost
            for child in cur_node.successors():
                child.compute_cost()
